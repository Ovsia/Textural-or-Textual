...
def train( model,Origin,Nons,Cons,Typo):
    ...
    for epoch in range(num_epochs):
        ...
        for i,(data, data_cons,data_typo,data_nonsense) in enumerate(zip(ImageNet100Origin,ImageNet100Cons,ImageNet100Typo,ImageNet100Nons)):
            ...
            typographic_image, label, catogery_typo, _, _,attack_text,_ = data_typo
            ...
            text2 = ["a photo of the word " + catogery_origin[i] + " written over a picture of a " + catogery_origin[i] for i in
                     range(len(catogery_origin))]

            text3 = ["a photo of the word " + attack_text[i] + " written over a picture of a " + catogery_typo[i] for i in
                     range(len(catogery_typo))]


            text4 = ["a photo of the nonsense word"  + " written over a picture of a " + catogery_nonsense[i] for i in
                     range(len(catogery_nonsense))]
